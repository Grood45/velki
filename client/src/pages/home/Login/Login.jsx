...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const loginData = await loginUser({ username, password }).unwrap();
      ...
      if (loginData.token) {
        ...
        if (userData?.role && userData.role !== "user") {
          addToast("Admins must login through their specific portal.", {
            appearance: "error",
            autoDismiss: true,
          });
          return;
      ...
      <div className="w-full sm:p-6 text-[#6F8898]">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex items-center justify-center">
            <Button
              type="submit"
              className={`bg-[#ffc800] text-black w-1/3 text-base py-6 ${isLoginDisabled ? "opacity-50 cursor-not-allowed" : ""
                }`}
